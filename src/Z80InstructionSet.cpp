...
{
	std::string u = toUpper(s);
	return u == "A" || u == "B" || u == "C" || u == "D" || u == "E" || u == "H" || u == "L" || u == "F";
}
...
{
	std::string u = toUpper(s);
	return u == "BC" || u == "DE" || u == "HL" || u == "SP" || u == "AF" || u == "IX" || u == "IY";
}
...
std::vector<unsigned char> Z80InstructionSet::assembleLD(const std::string& op)
{
	if (op.find(',') != std::string::npos)
	{
		...
		if (isReg16(dst) && !isReg16(src) && src.find('(') == std::string::npos)
		{
			...
			return {opcode,static_cast<unsigned char>(val & 0xFF),static_cast<unsigned char>((val >> 8) & 0xFF)};
		}
